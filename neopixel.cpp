...
// This should be the D-Pin on Port 2
// of a JeeNode (which is digital pin 5 on an Uno board = ATmega328)
// http://www.fiz-ix.com/2013/02/direct-control-of-arduino-uno-digital-inputoutput-pins-using-port-registers/
...
void colorWipe(unsigned char r , unsigned char g, unsigned char b, unsigned wait ) {
  for(unsigned int i=0; i<PIXELS; i+= (PIXELS/4) ) {
...
#define THEATER_SPACING (PIXELS/2)

void theaterChase( unsigned char r , unsigned char g, unsigned char b, unsigned wait ) {
...
void rainbowCycle(unsigned frames , unsigned int frameAdvance, unsigned int pixelAdvance ) {
